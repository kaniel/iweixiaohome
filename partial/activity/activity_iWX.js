angular.module('iwx').controller('ActivityIwxCtrl', function ($scope, $http, $state, ngTableParams, $modal, eventType, $rootScope) {
    ...
    $scope.confirmModal = function(){
        ...
        } else if ($scope.confirm.type === 'unpublish_activity') {
            $http
                .post('/api/iwx/' + $scope.confirm.param + '/unpublish')
                .success(function () {
                    $scope.tableParams.reload();
                });
        }
    ...
    $scope.unpublish_activity = function (activity_id) {
        $scope.confirm.message = "您确定要对这个活动进行下架操作吗?";
        $scope.confirm.type = "unpublish_activity";
        $scope.confirm.param = activity_id;
        $("#confirmModal").modal();
        return;
    };
